...
### Fixed — Fail-closed verdict on tribunal collapse (LW-7, verdict-path safety)
- `orchestrator/tribunal_transversal.py`: a full tribunal collapse (every agent errored — connection error / BUDGET_EXCEEDED / parse-fail — so `agents_consulted==0`, the LW-5 signal) left `_deterministic_synthesis` with 0 findings, which fell to the severity table's empty branch and returned `SOLID UNDER PRESSURE` — the all-clear verdict — over zero analysis. A forensic auditor failing OPEN (green) on infrastructure collapse is the wrong-direction failure. LW-5/LW-6 had already made CONFIDENCE honest (LOW) and the escalation reason honest ("tribunal collapse") on collapse, but the `final_verdict` itself still failed open.
- New `_apply_collapse_guard(unified)` + `COLLAPSE_VERDICT = "INDETERMINATE — TRIBUNAL COLLAPSE"`, called in `run()` after `_maybe_escalate` (so it reflects final coverage). When `agents_consulted==0` the verdict is withheld: `final_verdict = INDETERMINATE — TRIBUNAL COLLAPSE` + a fail-closed reasoning ("rerun once agent connectivity is restored"). BINDING verdict-path gate, kept DISTINCT from the NON-BINDING confidence/escalation layers (which never touch `final_verdict`). It CANNOT mask a real adverse verdict: `>=1 FATAL -> INVIABLE` requires `>=1` contributing agent, so the guard only ever fires over a genuinely empty analysis. Idempotent.
- `orchestrator/schema.py`: `UnifiedVerdictOutput.final_verdict` description documents the new state (doc-only).

### Tests
- `orchestrator/smoke_test_e2e.py`: new `f_collapse_fail_closed` — all-error outputs -> `_deterministic_synthesis` 0 findings -> `_apply_confidence` grounds `agents_consulted==0` -> the guard overrides `SOLID UNDER PRESSURE` to the collapse verdict; INVERSE: healthy coverage (the FATAL synthetic set) keeps `INVIABLE` untouched. `b_unified_output` hardened: a collapsed live run (0 agents) now SKIPs (did not exercise live synthesis) instead of a false PASS/FAIL — the harness must not fail-open on collapse either.

### Versioning
- Operator-visible orchestrator banners (main x2 / wizard / transparency report) -> v3.23.0 (bump_manual). Product-face (base + router + 19 variants + README + CLAUDE) -> v3.23.0 (bump_stamps). Module docstrings frozen at origin (schema/tribunal v3.0.0; main/wizard v3.10.0). No prompt/skill CONTENT, no roster (9 N2), no skill (7), no domain-count (20) change.

### Verdict-binding guarantee
- LW-7 is the FIRST orchestrator change that IS verdict-binding — it changes `final_verdict` (on collapse only). Prior collapse-path fixes (LW-5/LW-6) were NON-BINDING (confidence/escalation). The guard is a separate, explicit fail-closed gate: it never touches `compute_confidence`, `Finding`, or the severity table; the deterministic `>=1 FATAL -> INVIABLE` path is byte-identical, proven by `e_monotonic_verdict` + `inverse_INVIABLE_kept`. Regression: `smoke_test_e2e.py` OFFLINE GREEN (13 PASS, 1 SKIP = `b_unified_output` no key, 0 FAIL) incl. `f_collapse_fail_closed`.

### JARP_CERTIFIED: DS v3.23.0 — PA-20260712-002 ✅

Level 1 — JARP DEEP SUBSTANTIVE 7-axis forensic audit of `dark-strategist-agent` v3.23.0 by `prompt-architect-agent` v1.3.0 (PA-20260527-002), over the v3.22.0 baseline. Scope: v3.23.0 delta — fail-closed verdict on tribunal collapse (LW-7): `_apply_collapse_guard` + `COLLAPSE_VERDICT = "INDETERMINATE — TRIBUNAL COLLAPSE"` called in `run()` after escalation; a 100% collapse (`agents_consulted==0`) now withholds the verdict instead of returning the all-clear `SOLID UNDER PRESSURE`; `schema.py` final_verdict doc; `smoke_test_e2e.py` `f_collapse_fail_closed` added + `b_unified_output` collapse-aware SKIP; atomic §4.14.1 bump (product-face + operator banners main x2 / wizard / transparency report -> v3.23.0; module docstrings frozen). RULE 08 self-audit L0 (PA-20260712-001) PASS first. SUBSTANTIVE — NOT confirmatory: LW-7 is the first collapse-path change that is VERDICT-BINDING (it changes `final_verdict`), unlike the NON-BINDING LW-5/LW-6. Functional evidence on the real machine (post-apply + post-bump): `smoke_test_e2e.py` OFFLINE GREEN (0 FAIL, 1 SKIP = `b_unified_output`, no key) with `f_collapse_fail_closed` (collapse -> INDETERMINATE + inverse INVIABLE kept) + `e_monotonic_verdict` (>=1 FATAL -> INVIABLE) + `c_fallback_intact` + `r2_byo_corpus` PASS; PLUS a LIVE forced-collapse e2e (DS-8C9732B9, dead-backend forced 100% collapse, $0 — 0/40 calls): 10 agents connection-error -> 0 contributing -> `VEREDICTO: INDETERMINATE — TRIBUNAL COLLAPSE` + Confidence LOW + Escalation NO ("zero agent coverage -- tribunal collapse") + SSM safely inert ("verdict not recognized as qualifying") + banner v3.23.0 — the pre-fix baseline returned `SOLID UNDER PRESSURE` on this exact collapse. The guard is a BINDING verdict-path gate kept distinct from the NON-BINDING confidence/escalation layers: it writes only `final_verdict`/`verdict_reasoning` on `agents_consulted==0`; `compute_confidence`, `Finding`, `_apply_confidence`, and the severity table (`>=1 FATAL -> INVIABLE`) are byte-identical/untouched — it CANNOT mask a real INVIABLE (>=1 FATAL needs >=1 contributing agent). A6 coherence IMPROVED: verdict/reasoning/confidence/escalation/SSM now all agree on "collapse" (pre-fix, verdict SOLID contradicted confidence LOW). Forensic surface (19 variants + 7 skills + base + router CONTENT) byte-identical except stamps. No real-person impersonation; no prompt/skill change. Result: 0 CRITICAL | 0 SERIOUS | 0 MODERATE | 0 LATENT -> `JARP_CERTIFIED`. `BIAS_CHECK_RESULT: PASS` (deterministic coverage==0 fail-closed gate, orthogonal to content/domain/party; fail-closed is the safety-correct direction for a forensic auditor). SUBSTANTIVE re-cert. Supersedes PA-20260711-002 (DS v3.22.0). `JARP_BENCHMARK_LIVE` advances to v3.23.0. Valid until 10/10/2026 or DS v4.0.0. Backlog: LW-7 CLOSED; the cert-surface<->runtime gap finding (the 19 domain variant prompt files are NOT injected into the live N1 pipeline — enforcement relies on the base model's generic reasoning under a domain label + tool list) remains OPEN as a standalone architectural decision (fix injection vs re-scope cert), separate from this release.
