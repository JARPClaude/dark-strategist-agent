...
**⚠️ BORRAR en este cierre:** v31 (`git rm` en el commit de cierre). v32 = único continuity vigente. Subir v32 al PROYECTO claude.ai + quitar v31.
...
4. `git rm dark-strategist-continuity-prompt_v31.md`.
5. `jarp-toolkit` + `.claude-init.md` → v3.19.0/PA-20260612-002 vía **Ruta-3** (7 bloques: init header + #7; toolkit header + #30 Version/CERT STATUS/Previous-certs + #16×2). Verificado content-based: `.claude-init.md` re-leído OK; `JARP_TOOLKIT.md` confirmado por `Select-String`. Commit separado en repo jarp-toolkit.
6. **One-shot BORRADO** antes del commit: `bump_manual_v3_19_0.py` (DS).
7. **Verificación post-push:** AMBOS canónicos concuerdan en v3.19.0/PA-20260612-002 (drift-check OK). HEAD remoto confirmado vía `list_commits`.
...
- **v3.19.0 — CERTIFICADO (`PA-20260612-002`)**. Default `claude-opus-4-7`. HEAD = commit de cierre v3.19.0 (**registrar SHA tras push**). HEAD previo (inicio s33) = `5bb546e`.
- **MODIFICADO:** `orchestrator/tribunal_transversal.py` (`_apply_confidence` corroboración por similitud + banner v3.19.0); `orchestrator/config.example.json` (`corroboration_min_overlap: 4`). **NEW:** `orchestrator/test_apply_confidence.py` (16 checks).
...
- `smoke_test_e2e.py` + `smoke_contract.txt` GITIGNORADOS (local-only). `_livewatch/` throwaway no-commiteado (`lw2_contract.txt` + previos `signals_acme.txt`/`strategy_acme_board_proposal.txt`).

### Repo prompt-architect-agent
- v1.3.0 (`PA-20260527-002`) ACTIVE. Auditor de la re-cert s33. Sin cambios.

### Repo jarp-toolkit
- header + #30 (Version/CERT STATUS/Previous certs) + #16 + `.claude-init.md` header + #7 → v3.19.0/PA-20260612-002. PRIVADO. **AMBOS canónicos concuerdan (verificado al cierre).**
...
**Repo dark-strategist-agent (v3.19.0 cert) — commit atómico:**
```
